#!/bin/bash
SAMPLES=$(ls -1 /projects/f_geneva_1/alyssa/grahami/gatk/bwa/*.bam | cut -d "_" -f 1-5 | cut -d "/" -f 8 | sort | uniq)
for SAMPLE in $SAMPLES
  do
	BAMS=$(ls -1 /projects/f_geneva_1/alyssa/grahami/gatk/bwa/*${SAMPLE}*.bam | cut -d "_" -f 1-7 | cut -d "/" -f 8 | sort | uniq)
	for BAM in $BAMS
		do
		READS=$(ls -1 /projects/f_geneva_1/alyssa/grahami/${SAMPLE}_filtered.R1.fq.gz | cut -d "/" -f 8 | cut -d "." -f 1-3 | sort | uniq)
		for READ in $READS
			do
